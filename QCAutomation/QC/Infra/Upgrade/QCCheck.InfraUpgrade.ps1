...
if($RequestType -eq "QC")
{
    ...
    Describe 'Hierarchy Functionality Validation' {
        ...
        if($SiteCode -eq $PrimarySiteCode)
        {
            Context 'Validate if component status messages not giving any SUP component errors' {
                ...
                $ComponentStatusMessages = Get-ComponentStatusMessagesForSUP -SQLServerName $SiteSQLServerName -SQLDBName $SiteSQLDBName -SiteCode $SiteCode -DaysOld $DaysOld -IgnoreComponents $IgnoreComponents
    ...
    Describe 'Site Server availability Validation' {
        ...
        if(($SiteCode -eq $PrimarySiteCode) -or ($RequestType -eq "Pre Work") -or ($RequestType -eq "Post Work"))
        {
            Describe 'Replication Status' {
                Context 'Validate Site to Site communications' {

                    $DBRepolicationStatus = Get-ReplicationStatus -SiteCode $CentralSiteDetails.SiteCode -ProviderServerName $CentralSiteDetails.SiteServer
    ...
    }#describeSiteServerAvailabilityValidation

    Describe 'Infra Pre/Post work checks' {
        if($RequestType -eq "Pre Work") 
        {
            Context 'Suppress all the servers in ICM for 24 hours' {
                $SuppressResults = Perform-SuppressUnSuppress -SuppressionAction "Suppress"            
                It "Verify if all servers are Suppressed in ICM for 24 hours" -TestCases $SuppressResults.Output.SuppressionResult {
                    param
                    (
                        $SuppressionResult
                    )
                    $SuppressionResult | Should not be $null                
                }
            }
        }
   
        if(($RequestType -eq "Pre Work") -or ($RequestType -eq "Post Work"))
        {
            Context 'Incoming backlogs' {

                $InboxStateSyslogCount = Get-InboxStateSyslogCount -CentralSiteDetails $CentralSiteDetails            

                It "Verify the incoming state sys backlog - '<Name>' on '<PSComputerName>' count is '<Count>'" -TestCases $InboxStateSyslogCount.Output.result {
                    param
                    (
                        $Name,
                        $Count,
                    ...
                    $Count -le 50000 | Should Be $true               
                }
            }

            Context 'Export the Hardware inventory \Set classes settings on CAZ' {
                $Hardwareinventorysettings = Export-Hardwareinventorysettings -CentralSiteDetails $CentralSiteDetails -ExpectedUpgradeName $ExpectedUpgradeName -InputValues $InputValues
                It "Verify if able to export  Hardware inventory \Set classes settings on CAZ Path - '<File>'" -TestCases $Hardwareinventorysettings.Output.ExportHWInvClasses {
                    param
                    (
                        $File
                    )
        ...
        if($RequestType -eq "Pre Work")
        {
            Context 'Disk Space Details Of CAS and Primary Site Servers' {

                $DiskSpaceDetails = Get-DiskSpaceDetails -CentralSiteDetails $CentralSiteDetails

                It "Verify the Disk space details on - '<PSComputerName>' DeviceId '<DeviceID>' FreeSpace in GB's '<Freespace>'" -TestCases $DiskSpaceDetails.Output.details {
                    param
                    (
                        $DeviceID,
                        $Freespace,
                    ...
                    $DiskSpaceDetails | Should Not Be $null                
                }
            }

            Context 'Take Registry Backup on CAS and Primary' {

                $RegistryBackUp = Get-RegistryBackUp -CentralSiteDetails $CentralSiteDetails -ExpectedUpgradeName $ExpectedUpgradeName -InputValues $InputValues
                It "Take Registry Backup on - '<PSComputerName>' FileName '<RegFileName>' Path '<Destination>'" -TestCases $RegistryBackUp.Output.details {
                    param
                    (
                        $RegFileName,
                        $PSComputerName,
                    ...
                    $RegFileName | Should Not Be $null
                }
            }

            Context 'Reboot Pending on CAS and Primary' {

                $RebootPendingDetails = Get-RebootPendingDetails -CentralSiteDetails $CentralSiteDetails
            
                It "Verify No Reboot pending on - '<PSComputerName>'" -TestCases $RebootPendingDetails.Output.Rebootdetails {
                    param
                    (
                        $Rebootpending,
                        $PSComputerName
                    )
                    $Rebootpending | Should Not Be $true                
                }
            }

            Context 'AV exclusion on CAZ and primary ' {

                $AVExclusion = Add-AVExclusion -CentralSiteDetails $CentralSiteDetails            

                It "Verify if AV exclusion is added on  - '<PSComputerName>' ExclusionPath - '<ExclusionPath>'" -TestCases $AVExclusion.Output.AVExcusionDetails {
                    param
                    (
                        $ExclusionPath,
                        $PSComputerName
                    )
                    $ExclusionPath | Should Not Be $null
                }
            }

            Context 'Disable SQL jobs on all SQL nodes' {

                $DisableSQLjobs = Disable-SqlJobs -CentralSiteDetails $CentralSiteDetails -ExpectedUpgradeName $ExpectedUpgradeName
            
                It "Verify if SQL Jobs are disabled on - '<PSComputerName>' and DisabledJoblistPath - '<jobsfilepath>'" -TestCases $DisableSQLjobs.Output.JobDetails {
                    param
                    (
                        $jobsfilepath,
                        $PSComputerName,
                    ...
                    $serverInstance | Should Not Be $null
                }
            }

            Context 'AlwaysOn High Availability Status' {

                $AGdetails = Get-AGStatus -CentralSiteDetails $CentralSiteDetails

                It "Verify if AlwaysOn High Availability is '<synchronization_health>', and it should be in synchronized state - '<synchronization_health_desc>' on - '<PSComputerName>'" -TestCases $AGdetails.Output.Rundetails {
                    param
                    (
                    ...
                    $synchronization_health_desc | Should Be 'Healthy'
                }
            }

            Context 'Availability Replica Failover Mode  from Automatic to Manual' {

                $Failovermodedetails = Update-FailOverMode -CentralSiteDetails $CentralSiteDetails -FailoverMode "Manual"

                It "Verify if Availability Replica Failover Mode  is '<FailoverMode>' on - '<PSComputerName>'" -TestCases $Failovermodedetails.Output.queryFailoverMode {
                    param
                    (
                        $FailoverMode,
                        $PSComputerName
                    )
                    $FailoverMode | Should Be 'Manual'
                }
            }

            Context 'Remove Old DB backup on Primary SQL nodes' {

                $OldSqlBackupdetails = Delete-OldBackupFiles -CentralSiteDetails $CentralSiteDetails

                It "Verify if able to delete old DB backup on - '<PSComputerName>' " -TestCases $OldSqlBackupdetails.Output.OldBackup {
                    param
                    (
                    
                        $PSComputerName
                    )
                    $PSComputerName | Should not Be $null
                }
            }

            Context 'DB backup on Primary SQL nodes' {

                $SqlBackupdetails = Get-SqlBackup -CentralSiteDetails $CentralSiteDetails -ExpectedUpgradeName $ExpectedUpgradeName -SiteSQLListeners $SiteSQLListeners -SiteSQLDBNames $SiteSQLDBNames

                It "Verify if able to take DB backup on - '<PSComputerName>' path 'H:\SqlBackUP'" -TestCases $SqlBackupdetails.Output.Results {
                    param
                    (
                    
                        $PSComputerName
                    )
                    $SqlBackupdetails | Should not Be $null                
                }
            }

            Context 'Disable WSUS SYNC on CAZ' {

                $WSUSSyncSchedule = Disable-WSUSSyncSchedule -CentralSiteDetails $CentralSiteDetails

                It "Verify if able to Disable WSUS SYNC on CAZ " -TestCases $WSUSSyncSchedule.Output.DisableWSUSSyncSchedule {
                    param
                    (
                        $Schedule
                    
                    )
                    $Schedule | Should  Be "Not available"
                }
            }
        }
   
        if($RequestType -eq "Post Work")
        {
            Context 'Remove AV exclusion on CAZ and primary ' {

                $RAVExclusion = Remove-AVExclusion -CentralSiteDetails $CentralSiteDetails

                It "Verify if AV exclusion is Removed on  - '<PSComputerName>' ExclusionPath - '<ExclusionPath>'" -TestCases $RAVExclusion.Output.RemovedExcusionDetails {
                    param
                    (
                        $ExclusionPath,
                        $PSComputerName
            ...
            Context 'Availability Replica Failover Mode  from Manual to Automatic' {

                $Failovermodedetails = Update-FailOverMode -CentralSiteDetails $CentralSiteDetails -FailoverMode "Automatic"
                ...
                It "Verify if Availability Replica Failover Mode  is '<FailoverMode>' on - '<PSComputerName>'" -TestCases $Failovermodedetails.Output.queryFailoverMode {
                    ...
                    $FailoverMode | Should Be 'Automatic'
                }
            }

            Context 'Enable SQL jobs on all SQL nodes' {

                $EnableSqlJobs = Enable-SqlJobs -CentralSiteDetails $CentralSiteDetails -ExpectedUpgradeName $ExpectedUpgradeName

                It "Verify if SQL Jobs are enabled on - '<PSComputerName>'" -TestCases $EnableSqlJobs.Output.JobDetails {
                    param
                    (
                        $PSComputerName,
                        $serverInstance,
                    ...
                    $jobstatus | Should Be "Enabled"
                }
            }

            Context 'Enable WSUS SYNC on CAZ' {

                $WSUSSchedule = Enable-WSUSSyncSchedule -CentralSiteDetails $CentralSiteDetails

                It "Verify if able to enable WSUS Sync on CAZ " -TestCases $WSUSSchedule.Output.EnableWSUSSyncSchedule {
                    param
                    (
                        $Schedule,
                        $Task
                    )
                    $Schedule | Should not Be "Not available"
                }
            }
        }    
    }#InfraPre/PostWorkChecks
}
