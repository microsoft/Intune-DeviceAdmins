...
Import-LocalizedData -BaseDirectory $PSScriptRoot -FileName ClientHealthQCConfigurations.psd1 -BindingVariable Configurations
...
$TestResults = Invoke-Pester -Script @{Path="$ScriptRoot\QCCheck.ClientHealthUpgrade.ps1";Parameters=@{Configurations=$Configurations}} -PassThru -ErrorAction SilentlyContinue 
$QCEndTime = [DateTime]::Now
$QCTitle = "ClientHealth QC result for Client Machines" -f $InputParams["UpgradeDateTime"]
