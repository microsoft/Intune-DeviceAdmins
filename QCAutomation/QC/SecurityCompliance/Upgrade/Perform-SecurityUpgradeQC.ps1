...
$DPGroupName = $ConfigParams.DPGroupName
$CollectionName = $ConfigParams.CollectionName
$DeployPackageLocation = $ConfigParams.DeployPackageLocation
...
$FieldSettings = @()
$FieldSettings += New-FieldSetting -FieldOrder 1 -FieldName "QCTitle" -DisplayName "QC Title" -Description "Please enter appropriate QC title. (e.g. 1812_Security_PreUpgrade or 1812_Security_PostUpgrade)" -DataType Text -IsVisible $true -DisplayWidth 200 -FieldWidth 200 -Mandatory $true
...
$FieldSettings += New-FieldSetting -FieldOrder 6 -FieldName "DPGroupName" -DisplayName "DPGroupName" -Description "Please enter valid DP GroupName to Deploy Package." -DataType Text -IsVisible $true -DisplayWidth 200 -FieldWidth 200 -Mandatory $true -DefaultValue $DPGroupName
$FieldSettings += New-FieldSetting -FieldOrder 7 -FieldName "CollectionName" -DisplayName "CollectionName" -Description "Please enter valid Collection Name to distrubute ADR." -DataType Text -IsVisible $true -DisplayWidth 200 -FieldWidth 200 -Mandatory $true -DefaultValue $CollectionName
$FieldSettings += New-FieldSetting -FieldOrder 8 -FieldName "ADRDescription" -DisplayName "ADRDescription" -Description "Please enter valid ADR Description." -DataType Text -IsVisible $true -DisplayWidth 200 -FieldWidth 200 -Mandatory $true -DefaultValue "1810 QC by Automation"
$FieldSettings += New-FieldSetting -FieldOrder 9 -FieldName "ArticleId" -DisplayName "ArticleId" -Description "Please enter valid ArticleId." -DataType Text -IsVisible $true -DisplayWidth 200 -FieldWidth 200 -Mandatory $true -DefaultValue '4041678'
$FieldSettings += New-FieldSetting -FieldOrder 10 -FieldName "DeployPackageLocation" -DisplayName "DeployPackageLocation" -Description "Please enter valid existing Source Path to create Package." -DataType Text -IsVisible $true -DisplayWidth 200 -FieldWidth 200 -Mandatory $true -DefaultValue $DeployPackageLocation


$WindowTitle = "Security and Compliance upgrade QC inputs"
...
#$TestResults = Invoke-Pester -Script @{Path="$ScriptRoot\QCCheck.AppsUpgrade.ps1";Parameters=@{Configurations=$Configurations}} -PassThru -ErrorAction SilentlyContinue 
$TestResults = Invoke-Pester -Script @{Path="$ScriptRoot\QCCheck.SecurityUpgrade.ps1";Parameters=@{InputParams = $InputParams; AllSiteArtifacts = $AllSites; Configurations = $Configurations}} -PassThru -ErrorAction SilentlyContinue
$QCEndTime = [DateTime]::Now
$Title = $InputParams.QCTitle
$QCTitle = "Security & Compliance QC result for site: {0} [{1}] UpgradeDate: {2}" -f $InputParams["SiteCode"], $SiteServer, $InputParams["UpgradeDateTime"]
